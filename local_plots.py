...
def main():
    ...
    comp_df = pd.read_csv("data/lookup_tweets.csv")
    ...
    # Complaints by date
    fig = px.histogram(
        comp_df, x="created_at", template="plotly_white", title="Complaint counts by date"
    )
    ...
    # Pre-processing data
    top_comps = (
        comp_df.groupby("company")["created_at"]
        .count()
    ...
    bigram_df = pd.read_csv("data/bigram_data.csv")
    fig = px.bar(
        bigram_df[:20],
        x="ngram",
        y="Total",
