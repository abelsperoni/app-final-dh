...
bigram_df = pd.read_csv("data/bigrams_counts_data.csv")
...
def calculate_bank_sample_data(dataframe, sample_size, time_values):
    ...
    if time_values is not None:
        ...
        dataframe = dataframe[
            (dataframe["created_at"] >= min_date)
            & (dataframe["created_at"] <= max_date)
        ]
    """
    company_counts = dataframe["company"].value_counts()
...
def plotly_wordcloud(data_frame):
    ...
    complaints_text = list(data_frame["text_clean"].dropna().values)
        ...
        html.A(
            ...
            dbc.Row(
                [
                    ...
                    dbc.Col(
                        dbc.NavbarBrand("Twitter Sentiment Analysis - Telefonia móvil en Argentina", className="ml-2")
                    ),
                ],
                align="center",
                no_gutters=True,
...
LEFT_COLUMN = dbc.Jumbotron(
    [
        html.H4(children="Filtros", className="display-5"),
        html.Hr(className="my-2"),
        html.Label("Elegí un porcentage del dataset", className="lead"),
        html.P(
            "(Menos es más rápido. Más es más preciso.)",
        ...
        html.Label("Elegí una compañía", style={"marginTop": 50}, className="lead"),
        html.P(
            "(Podés usar la lista o el gráfico de la derecha)",
            style={"fontSize": 10, "font-weight": "lighter"},
        ),
        dcc.Dropdown(
            id="bank-drop", clearable=False, style={"marginBottom": 50, "font-size": 12}
        ),
        html.Label("Elegí un periodo de tiempo", className="lead"),
...
WORDCLOUD_PLOTS = [
    dbc.CardHeader(html.H5("Palabras más usadas")),
    dbc.Alert(
...
TOP_BANKS_PLOT = [
    dbc.CardHeader(html.H5("Tweets por compañía")),
    dbc.CardBody(
        [
            dcc.Loading(
...
TOP_BIGRAM_PLOT = [
    dbc.CardHeader(html.H5("Tweets en el tiempo y por compañía")),
    dbc.CardBody(
        [
            dcc.Loading(
...
TOP_BIGRAM_COMPS = [
    dbc.CardHeader(html.H5("Comparación de bigramas de a dos compañías")),
    dbc.CardBody(
        [
            dcc.Loading(
                ...
                children=[
                    ...
                    dbc.Row(
                        [
                            dbc.Col(html.P("Elegí dos compañías para comparar:"), md=12),
                            dbc.Col(
                                [
                                    dcc.Dropdown(
                                        id="bigrams-comp_1",
...
def populate_bigram_scatter(value):
    top_comps_df = GLOBAL_DF
    ...
    fig.update_xaxes(categoryorder="category descending", title="Fecha").update_yaxes(
...
def comp_bigram_comparisons(comp_first, comp_second):
    ...
    fig = px.bar(
        temp_df,
        title="Comparación: " + comp_first + " | " + comp_second,
        x="ngram",
        y="value",
        color="company",
        template="plotly_white",
